...
export default function BenefitsSectionNonLanding() {
  const [heading, setHeading] = useState("Win more buybox");

  useEffect(() => {
    // Получаем параметр utp_content из URL
    const params = new URLSearchParams(window.location.search);
    const utpContent = params.get("utp_content");

    if (utpContent === "3") {
      setHeading("Always be first"); // Если параметр utp_content=3, изменяем заголовок
    }
  }, []);
